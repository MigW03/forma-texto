...
- [x] Pré-textual formatting (server-side, deterministic)
  ...
  - **Capa vertical centering (2026-07-02):** a real PDF export showed page breaks landing in the wrong place — the previous approach (`applyCoverYearBottom`, a fixed 5040-twip `spaceBefore` pushing the city/year down) assumed a content height that didn't hold for every capa, so a longer/shorter cover pushed the break onto the wrong page. Tried the textbook OOXML fix first — a section break on the capa with `<w:vAlign w:val="both"/>` (Word's native "justify vertically" page property) — but verified empirically against the real PDF export path (LibreOffice 26.2 headless) that it is **silently ignored**: rendered identically with `both`, `center`, and `bottom`, on both a mid-document section break and the document's own final section. LibreOffice DOES honor **table-cell** `vAlign`, though (confirmed empirically), so `applyCoverVerticalDistribution` (`preTextual.ts`) now collapses the capa's paragraphs into one borderless, full-page-height table cell with `vAlign="center"` — the cell's exact height is computed from the guideline's own page size/margins, so there is no constant to re-tune for different content lengths. This is a scoped middle ground: true ABNT distribution (institution top / title middle / city+year bottom) needs per-field classification, which is unbuilt (see the "detect and confirm" bucket in `business_decisions/pretextual-elements.html`) — centering the capa as one block is a real improvement over the old top-clumped bug without that larger feature. Because it collapses N blocks into 1, it must run **after** every block-index-dependent pass (Step C/D/P, sumário, captions/placeholders) — `processFormatting` now calls it last, re-detecting pré-textuais fresh at that point rather than reusing the early `pretextual` result. Verified end-to-end against a real LibreOffice PDF export (rendered page image inspected — content is centered, page count/pagination correct). 3 unit tests rewritten for the new implementation; server suite **350 green**. **Still open:** capa/folha full vertical *distribution* (3-zone top/middle/bottom, needs the detect-and-confirm field step) and folha de rosto's own vertical layout (still untouched by this pass).
  - **Real-document gap found (2026-07-02, same session, via a real thesis export):** `classifyPretextual`'s capa↔folha split requires **two** standalone year-lines in the leading region (`yearLines.length >= 2`) to conclude "this is two separate cover pages." A real document that puts city+year on only ONE combined block for the whole front matter (common — many students don't repeat city+year on both the capa and the folha de rosto) has only one year-line, so the whole region — capa text, any stray content the author left in between (in the reported case, a personal to-do-list page the author forgot to delete), and the folha de rosto — collapses into a single `folhaDeRosto` section. Because `applyCoverVerticalDistribution` only acts on a `capa`-kind section, it correctly no-ops here — but the practical effect is the new centering never applies to this (likely common) document shape, and city/year can end up isolated on its own page purely because of the author's own pre-existing page breaks within that merged region. Not fixed yet — a real fix needs to split the merged region by actual page boundaries (not year-line counting) and decide what to do with non-cover content caught inside it (e.g. a stray notes page), which is the same "field-level classification" problem already tracked as future "detect and confirm" work.
...
- [x] Sumário (ABNT table of contents) generation
  - Built differently than originally planned: instead of a native Word TOC field, `server/src/lib/formatting/sumario.ts` `buildSumario(documentXml, pretextual)` deterministically rebuilds the sumário section from the Heading1–3 paragraphs Step D produces — preserves the "SUMÁRIO" label, replaces its content blocks with one dot-leader TOC entry per body heading (H1 bold/flush-left, H2/H3 indented 709/1418 twips), deleting or appending blocks as the entry count differs from the old content. Runs after Step D in `processFormatting.ts`; `appendixStart` is recomputed afterward since the block count can change. Page numbers are intentionally left blank (dot-leader tab stop only, no number) — a real TOC field / pagination render pass is still future work.
  ...
  - **Still open:** real page numbers (needs the TOC field / pagination render pass, plus the coupled ABNT page-numbering prerequisite below); REFERÊNCIAS and other `ReferencesHeading`-styled sections (they use a different style than Heading1-3) are not yet scanned into the sumário.
