...
- [x] Appendix / annex (Apêndice / Anexo) exclusion — **detect and freeze, keep in file, don't bill**
  ...
  - **Detection** (`server/src/lib/formatting/postTextual.ts` → `locateAppendixStart`): scans for the first paragraph whose text opens with `Apêndice(s)`/`Anexo(s)` AND whose leading label is UPPERCASE (ABNT writes these titles in caps, which distinguishes a real heading from an in-body mention like "ver anexo A"). Returns the absolute block index, or null.
  ...
  - **Deferred (preview UI):** `PageSelectionPage` shows the appendix in the preview but draws no "Lauda N" divider over it (it is not a lauda). A future polish could visually mark it as a non-billable "frozen" section so the user understands why it isn't counted. **Limitation:** detection needs the heading typed in UPPERCASE; a title-case "Apêndice A" styled to caps via CSS/Word is not detected.
