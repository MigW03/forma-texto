...
- [x] Re-add document zoom controls to the lauda selection page (`PageSelectionPage.tsx`) — reuses the `ZoomControls` component from the project detail viewer. Working live.
- [x] Pré-textual element detection + classification (client-side, page selection)
  - `web/src/lib/pretextual.ts` (`detectPretextual`) deterministically locates the ABNT front-matter region (capa, folha de rosto, resumo, abstract, sumário, listas, agradecimentos, errata, …) from block text. Labeled sections via anchored regex; cover pages via position + year-line / natureza / orientador anchors. Returns `{ sections, bodyStart }`. `computeLaudas` gained a `bodyStart` arg so the pré-textual region is **excluded from laudas** (not billed), keeping absolute block indices. `PageSelectionPage` renders detected sections in a separate "Pre text elements" group at the top of the left panel (amber, non-selectable) and labels each section + the body's "Lauda 1" boundary in the center preview. i18n in all three locales (`pretextual.*`). Capa↔folha split is heuristic (year-line based) and the residual field-level confirm step is future work — see `business_decisions/pretextual-elements.html`.
  - **Slicing consistency:** lauda numbering must agree across page selection, checkout, and recovery. `laudas.ts` now exposes `analyzeDocument(blocks)` (detect + `computeLaudas(bodyStart)` in one) and `uploadKeepSet(blocks, selectedLaudas)` (always retains the pré-textual blocks so the front matter reaches the server even when a lauda subset is selected). `CheckoutPage` and `ProjectDetailPage` re-slice use these. Tested in `laudas.test.ts`.
- [x] Pré-textual formatting (server-side, deterministic)
  - `server/src/lib/formatting/preTextual.ts` mirrors the web detector on the document XML. `applyPretextualHeadings` stamps the ABNT unnumbered-title style (centered, bold, UPPERCASE — reuses `REFERENCES_HEADING_STYLE`, the same "título sem indicativo numérico" values) on each labeled heading (RESUMO/ABSTRACT/SUMÁRIO/listas/agradecimentos/errata/folha de aprovação). The orchestrator (`processFormatting`) runs it after Step B and passes `bodyStart` to Step D as `bodyStartIndex`, excluding the front matter so no cover/abstract line is promoted to a numbered heading. **Step P (proofreading):** `coverBlockIndices(sections)` (capa, folha de rosto, folha de aprovação) is passed as `excludeIndices` so the cover/identity pages are never "corrected" (names, institution, title), while resumo/abstract prose IS proofread normally — detected fresh in the Step P block so it also applies in proofreading-only mode. Tested: `preTextual.test.ts`, Step D + Step P exclusion cases (server suite 292 green). **Capa centering:** `applyCoverAlignment` stamps every capa paragraph with the new `CoverCentered` style (centered, no indent, body font/size; author run emphasis survives) — runs after Step B in the orchestrator. **Folha de rosto alignment:** `applyFolhaRostoAlignment` centers author/title/city/year and stamps `FolhaRostoNatureza` (8cm right half-indent, left-aligned) on the "apresentada como requisito parcial…" + orientador block per NBR 14724. New `FolhaRostoNatureza` paragraph style in `rewriteStyles.ts`. **Section page breaks:** `applyPretextualPageBreaks` injects `<w:pageBreakBefore/>` into the first paragraph of every pré-textual section except the first (ABNT: each element on its own page). Idempotent; composes correctly with heading style. Server suite **307 green**.
  - **Capa vertical centering (2026-07-02), extended to center + foot distribution (2026-07-06):** a real PDF export showed page breaks landing in the wrong place — the previous approach (`applyCoverYearBottom`, a fixed 5040-twip `spaceBefore` pushing the city/year down) assumed a content height that didn't hold for every capa, so a longer/shorter cover pushed the break onto the wrong page. Tried the textbook OOXML fix first — a section break on the capa with `<w:vAlign w:val="both"/>` (Word's native "justify vertically" page property) — but verified empirically against the real PDF export path (LibreOffice 26.2 headless) that it is **silently ignored**: rendered identically with `both`, `center`, and `bottom`, on both a mid-document section break and the document's own final section. LibreOffice DOES honor **table-cell** `vAlign`, though (confirmed empirically), so `applyCoverVerticalDistribution` (`preTextual.ts`) collapses the cover paragraphs into a borderless page-geometry table. **As of 2026-07-06 it does a two-zone layout on BOTH capa and folha de rosto:** each cover page becomes a main row (`vAlign="center"`, the institution/author/title block centered) plus — when the page's trailing lines are a city/year block (bare year line, optional title-case city line above it, or a combined "Recife – 2024" line) — a foot row (`vAlign="bottom"`) that pins the city/date to the page's last lines per NBR 14724. Each section is first split into the author's own pages (`pageBreakBefore` / `<w:br w:type="page"/>` signals), so the merged single-year-line document shape (capa+folha collapsed into one `folhaDeRosto` section) still gets a centered zone + pinned foot on every one of its real pages. Contiguous capa+folha sections merge into ONE `<w:tbl>` (adjacent tables would merge unpredictably in Word); row heights sum to the page content height so pagination is enforced by the rows themselves; page-break artifacts are stripped from paragraphs moving into cells. Because it collapses N blocks into 1, it must run **after** every block-index-dependent pass (Step C/D/P, sumário, captions/placeholders) — `processFormatting` calls it near the end, re-detecting pré-textuais fresh. Verified end-to-end against a real LibreOffice PDF export (rendered page images inspected — capa AND folha centered, city/year on the last lines of both pages, natureza offset intact, page count correct). True 3-zone ABNT distribution (institution top / title middle) still needs the detect-and-confirm field step.
  - **Real-document gap (2026-07-02), largely mitigated (2026-07-06):** `classifyPretextual`'s capa↔folha split requires **two** standalone year-lines to treat the region as two cover pages, so a document with one combined city+year block collapses everything into a single `folhaDeRosto` section. The distribution pass now handles that shape: it splits the merged section into page groups by the author's own page breaks and formats each page independently (center + foot), so city/year no longer strands mid-page or on its own page. Still open for the future field-level "detect and confirm" work: the *classification* split itself (capa vs folha vs stray content labels) and the 3-zone layout.
- [x] Caption detection tolerates a blank-line gap to the image
  - Authors often leave a blank line between the "Figura N — …" caption (or the "Fonte: …" source) and the image, so the strict-adjacency check missed it and the placeholder pass inserted a spurious red "[inserir legenda da figura]". `captions.ts` `nearestCaptionLine` now skips up to `MAX_CAPTION_GAP` (3) blank paragraphs (never real text, never across a table/another image) when looking above/below an image; `formatCaptions` styles the windowed caption/source, and `missingInputs.ts` `captionPresent` uses the same window so no placeholder is inserted when the caption/source is a blank line away. Tested in `captions.test.ts` + `missingInputs.test.ts` (server suite 296 green). The blank line itself is left in place (not closed) — pulling the caption adjacent is a possible follow-up.
- [x] Multi-line caption detection
  - Some captions span multiple paragraphs (label + one or two continuation lines above the image). The previous scan only grabbed the first non-blank paragraph above the image; if that was a continuation line instead of the "Figura N —" label, the label was missed. New `findCaptionsAbove(blocks, imageIdx, stopAt)` (exported from `captions.ts`) uses a 3-phase scan: Phase 1 skips up to `MAX_CAPTION_GAP` blank paragraphs to find the first text; Phase 2 returns immediately if it's the figure label; Phase 3 scans up to `MAX_CONTINUATION_LINES` (2) additional consecutive non-blank paragraphs looking for the label, stopping at a blank line, another image, or a table. Returns all indices in document order; `formatCaptions` stamps each as `Caption`. New tests: multi-line caption tagged; image hard-barrier prevents label bleed from previous figure; continuation count exceeded → no tag; blank between continuations stops scan. Server suite **311 green** (up from 296).
- [x] Sumário heading duplicate fix (blockText tab, isBodyHeading, Step D TOC filter, suppressFirstHeadingPageBreak)
  - Word auto-TOC entries (`<w:t>1 INTRODUÇÃO</w:t><w:tab/><w:t>5</w:t>`) had the tab silently dropped by `blockText`, producing `"1 INTRODUÇÃO5"` which passed `isBodyHeading`'s digit-prefix check and set `bodyStart` inside the sumário — Step D then saw TOC entries and classified them as Heading1, creating duplicate headings alongside the real body headings. Fixed in four layers: (1) `blockText` (`blocks.ts`) now replaces `<w:tab/>` with a space before extracting `<w:t>` runs, so `"1 INTRODUÇÃO 5"` passes `isTocEntry`'s trailing-space-digit pattern; (2) `isBodyHeading` (`preTextual.ts` + `pretextual.ts`) had a `^introdu[çc][ãa]o$` special case that fired on manually-typed sumário entries — removed; (3) `chunkHeadings` (`stepD.ts`) explicitly filters out `TOC1`/`TOC \d`-styled paragraphs as defense-in-depth even if `bodyStart` slips; (4) `suppressFirstHeadingPageBreak` (`processFormatting.ts`) is now skipped when pré-textuais exist (`pretextual.bodyStart === 0`), so INTRODUÇÃO keeps its own page break. Server suite green.
- [x] Scroll to placeholder on textarea focus
  - When a `needs_input` project shows the pending-input panel, clicking a textarea now scrolls the DOCX preview to the corresponding red placeholder paragraph. `DocxViewer` was converted to `forwardRef`, exposing `scrollToPlaceholder(allPending, target)` via `useImperativeHandle`. The algorithm sorts all inputs by `insertedAt` (document order), walks the preview DOM with a `TreeWalker` collecting red text nodes matching the PLACEHOLDER_TEXTS set, and calls `scrollIntoView({ behavior:'smooth', block:'center' })` on the element at the target's position. Each pending-input textarea fires `onFocus` to trigger the scroll. `docxViewerRef` wired in the parent `ProjectDetailPage`.
...
- [x] DOCX correction function (proofreading)
  ...
  - **Split-retry resilience on ALL AI passes + escalated last try (2026-07-06):** Step D and Step C used to lose their ENTIRE pass when a single chunk threw (only Step P had `proofreadResilient`) — the dominant real-world failure being a reasoning model burning its token budget on chain-of-thought and emitting no JSON (`finishReason: length`). All three passes now isolate a failing chunk, split it in half, and retry each half as its own AI call, recursing down to single blocks/entries. A single block that still fails gets ONE **escalated** retry — the chunk carries `escalated: true` and the decider drops the OpenRouter reasoning effort to `minimal` (thinking less is the one knob that changes an identical retry's outcome) — and is then skipped, keeping its deterministic result. Model-agnostic by design: the mechanism assumes nothing about which model runs the pass. Each pass now returns `failedIndices` and `processFormatting` logs `N block(s)/entr(ies) failed every retry` so partial degradation is visible in the job log instead of silent. 6 new tests across `stepD/stepC/stepProofread.test.ts`.
  ...
  - **Per-step model selection (2026-06-16):** each AI pass can now run on a different model — a single doc can use a strong model for heading classification (Step D) and a cheaper/faster one for proofreading (Step P). `AiConfig` gained `headingModel` / `referenceModel` / `proofreadModel`, each resolved from `AI_HEADING_MODEL` / `AI_REFERENCES_MODEL` / `AI_PROOFREAD_MODEL` and falling back to `AI_MODEL` when unset; the three deciders and the `processFormatting` "calling model" logs use their step model. Live `.env`: Step D (headings) on `nemotron-3-ultra-550b-a55b:free`, Step C (references) on `nemotron-3-super-120b-a12b:free`, Step P overridden to `nemotron-3-nano-30b-a3b:free`. 2 new config tests; documented in `.env.example`. **Note:** do not use nano for Step C — it over-reasons and corrupts JSON output (7k+ reasoning tokens, malformed JSON, `NoObjectGeneratedError` — incidents 2026-06-19/06-20). **Super (120b) is enough for Step C references; only Step D heading classification actually needs ultra (550b).**
  - **Step C JSON repair hardening (2026-06-19):** `headingDecider.ts` now has `sanitizeControlChars()` which is called at entry of `repairDecisions` — escapes `U+0000–U+001F` inside JSON string values before any parse attempt, so the entire repair chain works on clean text even when a reasoning model embeds a literal newline in its output. Fixes both Step C and Step D (the latter imports `repairDecisions`). `AI_REFERENCES_MODEL` set to `nemotron-3-super-120b-a12b:free` in `.env` — super is enough for references; ultra is reserved for Step D heading classification.
...
- [x] Appendix / annex (Apêndice / Anexo) exclusion — **detect and freeze, keep in file, don't bill**
  - ABNT post-textual sections (`APÊNDICE`, `ANEXO`) must not be proofread or reformatted — an appendix is the author's own supporting material and an annex reproduces third-party documents whose layout must survive byte-for-byte — but they must still ship in the downloaded file, and the user must not be charged for them.
  - **Detection** (`server/src/lib/formatting/postTextual.ts` → `locateAppendixStart`): scans for the first paragraph that is a `Apêndice(s)`/`Anexo(s)` heading. The pattern is **anchored at both ends** — the whole paragraph must be the label + optional enumerator (A, B, 1, II…) + optional "— Título" — so an in-body mention ("o anexo A contém os formulários") never matches. Because of that anchoring it is **case-insensitive**, matching every casing: `APÊNDICE A`, `Apêndice A`, `anexo i: mapa`, etc. Returns the absolute block index, or null.
  - **Freeze (server pipeline):** the index is a cutoff threaded into every per-block pass — `stripDirectOverrides` (via `applyStepA`), Step D (`refStartIndex`), `formatImages`, `formatCaptions`, `detectAndInsertPlaceholders`, `applyPunctNorm` (Step Punct), and Step P (`refStartIndex`). Blocks at/after the cutoff are never touched, but never removed, so the section is intact in the output `.docx`. The cutoff is recomputed before the proofreading passes because placeholder insertion can shift indices. `processFormatting` logs `appendix/annex detected at block N — section frozen`.
  - **Billing (frontend, `web/src/lib/laudas.ts`):** `computeLaudas` stops at the appendix boundary (`findAppendixBlock`, same uppercase heuristic), so appendix blocks are not billable laudas and the price drops automatically everywhere `getLaudas`/`computeLaudas` feed the count. `laudaBlockSet` keeps the appendix range unconditionally when the block list is passed, so the section survives slicing into the upload even when the user selects only a subset of laudas (`CheckoutPage` updated to pass the blocks).
...
- [x] Sumário (ABNT table of contents) generation
  ...
  - **Follow-up fixes (reported via screenshot: entries wrapping/overflowing the page, and a full body paragraph appearing as a TOC entry):** (1) `buildTocEntry` now explicitly resets `w:jc="left"` and `w:firstLine="0"` on every entry — it previously emitted no `w:pStyle` at all, so entries inherited the ABNT body style (justified, 1.25cm first-line indent) and wrapped/overflowed. (2) New deterministic pass `demoteImplausibleHeadings` (`server/src/lib/formatting/headingSanity.ts`), run right after Step B/before Step D, clears the Title/Heading style off any paragraph whose text exceeds `MAX_HEADING_CHARS` (200, `blocks.ts`) — this corrects an authoring mistake (forgetting to switch back to Normal after a subheading) that otherwise rendered a whole body paragraph as an oversized "heading" and leaked it into the sumário. Step D itself only ever *promotes* headings by design, never demotes, so this was the only place that could fix it. `buildSumario` keeps the same length guard as a second line of defense. Tested: `sumario.test.ts`, `headingSanity.test.ts`.
  - **Real-document follow-up (2026-07-02, from a real thesis export):** a *short* (under `MAX_HEADING_CHARS`) descriptive sentence right before an image's "Figura N:" label (e.g. "Com 800 metros quadrados, este painel é o maior da série…") was left in a Heading3 style, so the length guard never caught it — it leaked into the sumário as a bogus TOC entry, and rendered as an out-of-place bold "heading" stranded on its own page while the actual figure/label/source group moved to the next page (read by the user as "images separated from their caption"). `demoteImplausibleHeadings` gained a second, independent signal: a heading-styled paragraph whose very next non-blank block (tolerating one blank spacer) is a figure/table caption label (`FIGURE_LABEL_RE`/`TABLE_LABEL_RE`, reused from `captions.ts`) or an image itself is demoted regardless of length — a real chapter/section heading is essentially never placed with zero body prose between it and a caption label. +5 tests in `headingSanity.test.ts` (11 total).
  - **Ampersand double-escaping bug (2026-07-02, same real document):** a heading titled "CESAR & LOIS" rendered in the sumário as the literal text "CESAR &amp; LOIS". Root cause: `blockText` (`blocks.ts`) extracted `<w:t>` inner content **without decoding XML entities** — a literal "&" is always stored as `&amp;` in valid XML, so `blockText` returned the raw 5-character string `&amp;`, and `buildTocEntry`'s `escapeXml(text)` then re-escaped that `&`, producing `&amp;amp;` (renders as literal `&amp;`). Fixed at the source: new `decodeXmlEntities` in `xmlText.ts` (decodes named entities + numeric refs, `&amp;` last so it's never re-interpreted); `blockText` now decodes before returning. This is a general correctness fix for every consumer of `blockText`, not sumário-specific. +4 tests in `xmlText.test.ts`, +1 in `blocks.test.ts`. Server suite **359 green**.
  - **Still open:** REFERÊNCIAS and other `ReferencesHeading`-styled sections (they use a different style than Heading1-3) are not yet scanned into the sumário; ABNT header page-numbering (and the coupled folha-counting adjustment to the sumário numbers).

- [ ] Missing-file recovery flow (paid order, but `original_file_path` is null)
  - **Why it happens:** `handleSuccess()` in `CheckoutPage.tsx` runs *after* payment and reads the upload from `file-store.ts`, a volatile module-level variable. A full page reload (PIX / 3DS redirect, or a manual refresh) wipes it, so `getStoredFile()` returns null, the upload is skipped, and the project row is still inserted with `original_file_path: null`. `processFormatting` then aborts at its guard with "has no original_file_path". The order/payment succeeded, so the user has paid but we have no file.
  - **Flow to build:** in `processFormatting` (or `/api/processing/start`), when the project exists, payment/order is valid, but `original_file_path` is null → do **not** fail silently. Send the user an email right away asking them to re-upload the file (link to the project), and provide a path/endpoint that accepts the re-uploaded file, writes it to Storage, stamps `original_file_path` on the row, and re-triggers processing. **Never charge again** — this is our internal mistake (volatile in-memory file), not a new order. Guard against duplicate recovery emails (e.g. a `recovery_email_sent_at` column). Needs: a new email template (`server/src/emails/`), a re-upload UI on `ProjectDetailPage` (gated on `status === 'pending' && original_file_path == null`), and a server route to receive the file. Root-cause fix (upload before payment + persist the storage path string in `sessionStorage`) is deferred — this recovery flow is the safety net.
...
- [x] File auto-deletion cron job
  - Built server-side. `server/src/lib/cleanupExpiredFiles.ts` queries `projects` where `delete_files_at < now()` and `files_deleted_at is null`, removes each project's `original_file_path`, `processed_file_path`, and the derived `.pdf` from the `projects` storage bucket, then stamps `files_deleted_at` (rows kept; a storage failure leaves the row unstamped so the next run retries). Exposed via `POST /api/maintenance/cleanup-expired` (`routes/maintenance.ts`, `x-webhook-secret`). **Scheduled by Supabase pg_cron** (`server/sql/cleanup_cron.sql`): a daily `pg_net.http_post` hits the endpoint with the secret read from Vault — a real scheduler (survives restarts), keeping the tested TS logic with no Deno/SQL duplication. (Chose this over a Deno Edge Function: the server already can't be serverless — LibreOffice PDF export — so decoupling from it gains little, and over an in-process timer, which dies/resets on every deploy.) 8 unit tests (injectable fake client). **Run `cleanup_cron.sql` once per environment + set the two Vault secrets.**
...
- [x] File auto-deletion cron job
  - Done — see "File auto-deletion cron job" under Next Steps above. `cleanupExpiredFiles.ts` + `POST /api/maintenance/cleanup-expired`, scheduled by Supabase **pg_cron** (`server/sql/cleanup_cron.sql`, daily `pg_net.http_post` → endpoint, secret from Vault). Deletes original + processed `.docx` + the `.pdf`, stamps `files_deleted_at`. **Deploy step:** run `cleanup_cron.sql` once per environment and create the two Vault secrets (endpoint URL + webhook secret).
