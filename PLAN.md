...
- [x] Email notification when a project needs input *(done 2026-06-17)*
  - When the pipeline stamps `status = needs_input` (missing figure/table captions or sources), it now also emails the user — same shape as the ready email but pointing them to fill in the placeholders. `sendProjectNeedsInputEmail` in `server/src/lib/notify.ts` (looks up project + user email via service role, sends via Resend), template `server/src/emails/projectNeedsInput.ts`, called non-fatally from `processFormatting` step 7's `needs_input` branch. Only fires on the pipeline's first entry into `needs_input`; partial fills via `/fill-content` don't re-notify, and the ready email still fires when the last slot is resolved.
...
- [x] **Detection pass — find missing caption/source slots** *(done 2026-06-17)*
  - `server/src/lib/formatting/missingInputs.ts` — `detectAndInsertPlaceholders`. Walks image (`<w:drawing>`/`<w:pict>`/`<w:object>`) and `<w:tbl>` anchors, checks i-1 (caption slot) and i+1 (source slot) using `FIGURE_LABEL_RE`, `TABLE_LABEL_RE`, `SOURCE_LABEL_RE`, and Caption-style detection. Slot is occupied if the neighbour has Caption style OR matching label text. Unstyled-but-labelled neighbours are styled (no pending entry). Stacked images/tables are skipped conservatively (no placeholder between them).

- [x] **Placeholder insertion — pre-formatted, red, machine-locatable** *(done 2026-06-17)*
  - `buildPlaceholderXml()` produces a Caption-style `<w:p>` with `<w:color w:val="FF0000"/>`. `insertAroundBlocks()` does one BLOCK_RE pass to inject before/after. `insertedAt` is tracked via a cumulative-offset algorithm so the stored index stays valid in the final DOCX. 18 unit tests in `missingInputs.test.ts`.

- [x] **New project status `needs_input`** *(done 2026-06-17)*
  - `web/src/lib/status.ts` and `badge.tsx` updated. Orange badge variant. Dashboard `activeCount` and DashboardPage include it. All three locale files have `dashboard.status.needs_input`.

- [x] **Persist the pending slots — `projects.pending_inputs` (jsonb)** *(done 2026-06-17)*
  - Column documented in `supabase_tables.md`. Migration SQL: `ALTER TABLE projects ADD COLUMN pending_inputs jsonb; ALTER TABLE projects ADD COLUMN removed_inputs jsonb;`

- [x] **Pipeline wiring — stamp `needs_input` vs `complete`** *(done 2026-06-17)*
  - `processFormatting.ts` step 7 branches on `pending.length`: `needs_input + pending_inputs` (no email) or `complete + pending_inputs: null` (+ email). `pending` scoped before `if (doFormatting)` so it's reachable at step 7.

- [x] **Fill-in endpoint — `POST /api/processing/fill-content`** *(done 2026-06-17)*
  - `server/src/routes/processing.ts`. Bearer auth via `authorize()`. Accepts `{ projectId, fills?, removals? }`. Downloads/transforms/re-uploads. Partial fills supported. `removed_inputs` appended (legal audit trail). Flips to `complete` + sends ready email when no pending slots remain.

- [x] **Frontend — fill-in UI in the processed-file view** *(done 2026-06-17)*
  - `ProjectDetailPage.tsx`: queries `pending_inputs`, viewer opens processed file for `needs_input` projects (download disabled until `complete`), floating input overlays inside `DocxViewer` scroll container aligned with red placeholder text nodes (via `getBoundingClientRect` scan after `onRendered`), per-input save + remove buttons, remove confirmation modal via `createPortal`. Three locale files have `project.fillIn.*` keys.

- [x] **Tests** *(done 2026-06-17)*
  - 18 server unit tests in `missingInputs.test.ts` covering detection, insertedAt computation, fill, remove, ordinals, and edge cases. Server suite 167 passing, `tsc` clean both sides.

- [x] **Overlay placement + interaction performance** *(done 2026-06-17)*
  - Overlays now sit fully in the **right gutter, outside the page**: `scanPlaceholders` reads the `.docx-wrapper` right edge (relative to the scroll container, scroll-adjusted) and positions each input at that x + 12px, instead of the old `right: 12` that overlapped the rendered page at higher zoom. Interaction no longer re-renders the whole document — save/remove patch the rendered DOM in place (`placeholderEls` ref: replace the red run's text / drop the color on fill, remove the closest `<p>` on remove) and the realtime handler skips re-signing the processed URL when the file path is unchanged (`signedProcPathRef`), so filling an input is near-instant instead of triggering a full `docx-preview` re-render each time. `refreshProcessedUrl` removed. Web suite 32 passing (stale `status.test.ts` updated for the `needs_input` variant), production build green.

- [x] **Fix: stale `insertedAt` after a removal** *(done 2026-06-17)*
  - A removal deletes one block, so every surviving pending input whose `insertedAt` was after it shifts up by one — but the endpoint persisted the survivors' **original** indices, so a second fill/remove on a multi-figure doc targeted the wrong block. New `shiftPendingAfterRemovals(remaining, removedInsertedAts)` in `missingInputs.ts` recomputes each survivor's index (subtract the count of removed blocks before it); `fill-content` applies it before writing `pending_inputs`. 4 unit tests. (Single-placeholder docs were unaffected — this only bit docs with ≥2 slots.)

- [x] **Fix: stale processed file after fill/remove (cache + reconcile)** *(done 2026-06-17)*
  - Symptom: removing a caption cleared it from the UI, but the exported file still had the red placeholder and a page reload brought it back. Root cause: the `needs_input` file is overwritten in place by `/fill-content`, but it was uploaded with Supabase Storage's default `cacheControl` (1h, keyed by path), and the client reused the **same signed URL** — so both the download and the post-reload re-render fetched a **stale cached copy** (the CDN keys by path and ignores the changing token). Fixes: (1) both processed-file uploads (`processFormatting` step 6 + `/fill-content`) now use `cacheControl: '0'`; (2) **every processed signed URL is cache-busted** with a one-shot `&_cb=<ts>` param (`bustCache`) at all three sign sites (initial load, realtime, reconcile) so a fetch never resolves to a stale CDN/browser copy; (3) the frontend **reconciles with server truth** once the background write queue drains — when an action completes the doc (or a write fails) it re-fetches `pending_inputs`/status and re-signs the processed URL, so the viewer/download show the real saved file; (4) the download button is gated on `bgSaving === 0`. Intermediate saves still skip the re-render (optimistic DOM), so the flow stays fast.
  - **Known caveat (follow-up):** `/fill-content` does not regenerate the PDF export, so a `needs_input → complete` project's `.pdf` still shows the placeholders. Harmless today (LibreOffice isn't installed, PDF export is a no-op) but must be addressed when PDF export goes live — regenerate the PDF in `/fill-content` after the final fill, or drop the stale PDF.

- [x] **Fix: "Download final file" was clickable while awaiting input** *(done 2026-06-17)*
  - `<Button asChild disabled>` renders the child `<a>`, and `disabled` is a no-op on an anchor — so the final-download button stayed clickable in `needs_input`. Now a genuinely disabled `<button>` is rendered when `!canDownloadProcessed` (no `asChild`), with a `downloadLockedHint` tooltip (3 locales); the real `<a download>` is rendered only when the project is complete and not mid-save.

- [x] **Surface background fill/remove failures** *(done 2026-06-17)*
  - Background saves previously failed silently (`console.error` only), so a failed write looked like "input not saving" (the reconcile re-added the pending input with no explanation). Now `callFillApi` includes the HTTP status in its error, `runInBackground` stores the message in `saveError`, and a dismissible red banner shows `project.fillIn.saveError` (3 locales) in the viewer top bar; the next save attempt clears it. The state still self-heals via reconcile so the user can retry. (Note: `/fill-content` is AI-free, so it is **not** affected by the OpenRouter daily rate limit that can fail the Step C/D/P passes.)

- [x] **Background (non-blocking) fill/remove saves** *(done 2026-06-17)*
  - Saving an input no longer blocks the UI on the server round-trip (`fill-content` downloads → re-zips → re-uploads the whole DOCX each call). `handleSave`/`handleRemoveConfirm` now apply the change **optimistically** (DOM patch + local state + overlay/modal dismissal happen instantly) and push the API write onto a **serial background queue** (`saveQueue` ref) — serialized because each request reads-modifies-writes the same processed file, so concurrent writes would clobber each other (last upload wins). A small "Salvando alterações…" pill (`project.fillIn.savingBackground`, 3 locales) shows while `bgSaving > 0`. On a background failure the view self-heals via `reloadProject` (re-fetch `pending_inputs`/status + re-sign the processed URL to force a clean re-render). `savingId`/`removeConfirming` blocking states removed.
