...
- [ ] Sumário (ABNT table of contents) generation
  - Insert a native Word **TOC field** (`{ TOC \o "1-3" \h \z \u }`) built from the heading styles Step D already produces — we do **not** compute page numbers ourselves. Pagination is delegated to the rendering engine: LibreOffice fills the numbers on PDF export (already in our flow), and `<w:updateFields w:val="true"/>` makes Word update them on open (or LibreOffice can bake them into the `.docx`). Insert before the first body `Heading1` (sumário is the last pre-textual element). Coupled prerequisite: ABNT **page numbering** (PAGE field in header + section break so numbers show from the introduction) — the sumário numbers are meaningless without it. Planning only; no implementation yet.
