...
async function extractPageText(
  ...
  for (const item of content.items) {
    ...
    // item.height is the actual rendered font size in pdfjs v5
  ...
  // Merge adjacent lines of same font size into blocks
...
async function collectAllFontSizes(
  ...
  await Promise.all(
    pages.map(async (p) => {
      ...
      const content = await page.getTextContent()
      console.log('[extract] page', p, 'raw item count:', content.items.length)
      if (content.items.length > 0) console.log('[extract] first item sample:', JSON.stringify(content.items[0]))
      for (const item of content.items) {
        if (!('str' in item) || !item.str.trim()) continue
        const transform = item.transform as number[]
        const fontSize = Math.round((item.height || Math.abs(transform[3])) * 10) / 10
...
export default function TextExtractPage() {
  ...
  useEffect(() => {
    console.log('[extract] file from store:', file, 'state:', state)
    if (!file) {
        ...
        {!loading && !error && pages.map((pg) => (
          <div key={pg.pageNumber} className="mb-10">
            ...
            <div className="bg-white border border-border rounded-2xl px-8 py-8 flex flex-col gap-4">
              {pg.blocks.length === 0 && (
                <p className="text-sm text-muted italic">No text content found.</p>
              )}
