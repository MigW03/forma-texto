...
- ~~**Unnumbered-title style** (e.g. RESUMO, SUMÁRIO via `unnumberedTitle`) is specified in §8 but not yet applied in Step A.~~ **Resolved for labeled pré-textuais.** `preTextual.ts` (`detectPretextual` + `applyPretextualHeadings`) locates the front-matter region and stamps the references-heading style — the same "título sem indicativo numérico" values as `unnumberedTitle` (centered, bold, UPPERCASE) — on each labeled heading (RESUMO, ABSTRACT, SUMÁRIO, the listas, agradecimentos, errata, folha de aprovação). It also yields `bodyStart`, which the orchestrator passes to Step D (`bodyStartIndex`) so a cover/abstract line is never promoted to a numbered heading. The **capa** is now centered: `applyCoverAlignment` stamps every capa paragraph with the `CoverCentered` style (centered, no indent, body font/size — author run-level emphasis like a bold title survives). Folha de rosto is NOT centered here (its natureza note is offset to the right); full cover layout/generation still needs the detect-and-confirm field step (see `business_decisions/pretextual-elements.html`). **Step P** excludes the cover/identity pages (capa, folha de rosto, folha de aprovação, via `coverBlockIndices`) so names/institution/title are never "corrected"; resumo/abstract prose is still proofread.
